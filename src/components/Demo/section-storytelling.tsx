...
function StoryImage({ src, alt, className, onClick }: {
  src: string; alt: string; className?: string;
  onClick?: (e: React.MouseEvent<HTMLDivElement>) => void;
}) {
  return (
    <div className={`story-img relative overflow-hidden cursor-pointer group ${className ?? ""}`} onClick={onClick}>
      <div className="story-img-inner absolute inset-[-40px] transition-transform duration-700 group-hover:scale-105">
        <Image src={src} alt={alt} fill sizes="(max-width: 768px) 100vw, 80vw" className="object-cover" />
      </div>
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-500 flex items-center justify-center">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-12 h-12 text-white opacity-0 group-hover:opacity-80 transition-opacity duration-500 drop-shadow-lg">
...
export default function SectionStorytelling() {
  ...
  const [lightboxData, setLightboxData] = useState<(typeof STORY_CHAPTERS)[number] | null>(null);
  const [lightboxRect, setLightboxRect] = useState<DOMRect | null>(null);

  const openLightbox = useCallback((e: React.MouseEvent<HTMLDivElement>, idx: number) => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (chars?.length) {
        gsap.fromTo(chars,
          ...
          { y: 0, opacity: 1, rotationX: 0, duration: 3.5, ease: "power4.out", stagger: 0.08,
      ...
      if (heroSub) {
        gsap.fromTo(heroSub, { y: 50, opacity: 0 },
          { y: 0, opacity: 1, duration: 4, delay: 1.2, ease: "power2.out",
      ...
      sectionRef.current?.querySelectorAll(".hero-line").forEach((line) => {
        gsap.fromTo(line, { scaleX: 0 },
          { scaleX: 1, duration: 3.5, ease: "power2.inOut",
      ...
      if (heroStar) {
        gsap.fromTo(heroStar, { scale: 0, rotation: -360, opacity: 0 },
          { scale: 1, rotation: 0, opacity: 1, duration: 3.5, ease: "back.out(1.2)",
      ...
      if (ch1) {
        ...
        if (img1inner) gsap.fromTo(img1inner, { y: -60 }, { y: 60, ease: "none",
          scrollTrigger: { trigger: ch1, start: "top bottom", end: "bottom top", scrub: 3 } });
        const img1 = ch1.querySelector(".story-img");
        if (img1) gsap.fromTo(img1, { filter: "grayscale(1) sepia(0.6)" },
          { filter: "grayscale(0) sepia(0)", ease: "none",
            scrollTrigger: { trigger: ch1, start: "top 80%", end: "top 10%", scrub: 3 } });
        if (img1) gsap.fromTo(img1, { scale: 1.1, opacity: 0.7 },
      ...
      if (ch2) {
        ...
        if (tb2) gsap.fromTo(tb2, { x: -80, opacity: 0 },
          { x: 0, opacity: 1, duration: 3.5, ease: "power2.out",
            scrollTrigger: { trigger: ch2, start: "top 75%" } });
        const img2 = ch2.querySelector(".story-img");
        if (img2) gsap.fromTo(img2, { x: 120, rotation: 5, opacity: 0, scale: 0.9 },
          { x: 0, rotation: 0, opacity: 1, scale: 1, duration: 4, ease: "power3.out",
            scrollTrigger: { trigger: ch2, start: "top 72%" } });
        const img2inner = ch2.querySelector(".story-img-inner");
        if (img2inner) gsap.fromTo(img2inner, { y: -30 }, { y: 30, ease: "none",
          scrollTrigger: { trigger: ch2, start: "top bottom", end: "bottom top", scrub: 3 } });
        animTitleWords(ch2);
        animBody(ch2);
        animDecLine(ch2);
      }
      ...
      if (ch3) {
        ...
        if (img3) gsap.fromTo(img3,
          { clipPath: "inset(0 100% 0 0)", scale: 1.08, filter: "blur(8px)" },
          { clipPath: "inset(0 0% 0 0)", scale: 1, filter: "blur(0px)", duration: 4.5, ease: "power2.inOut",
            scrollTrigger: { trigger: ch3, start: "top 72%" } });
        const tb3 = ch3.querySelector(".ch-text-block");
        if (tb3) gsap.fromTo(tb3, { x: 80, opacity: 0 },
          { x: 0, opacity: 1, duration: 3.5, ease: "power2.out",
            scrollTrigger: { trigger: ch3, start: "top 70%" } });
        const img3inner = ch3.querySelector(".story-img-inner");
        if (img3inner) gsap.fromTo(img3inner, { y: -30 }, { y: 30, ease: "none",
          scrollTrigger: { trigger: ch3, start: "top bottom", end: "bottom top", scrub: 3 } });
        animTitleWords(ch3);
        animBody(ch3);
        animDecLine(ch3);
      }
      ...
      if (ch4) {
        ...
        if (img4) gsap.fromTo(img4, { filter: "grayscale(1) sepia(0.6)" },
          { filter: "grayscale(0) sepia(0)", ease: "none",
            scrollTrigger: { trigger: ch4, start: "top 60%", end: "bottom 40%", scrub: 3 } });
        const ov4 = ch4.querySelector(".ch4-overlay");
        if (ov4) gsap.fromTo(ov4, { opacity: 0, y: 60 },
        ...
        if (img4inner) gsap.fromTo(img4inner, { y: -40 }, { y: 40, ease: "none",
          scrollTrigger: { trigger: ch4, start: "top bottom", end: "bottom top", scrub: 3 } });
        animTitleWords(ch4);
      }
      ...
      if (ch5) {
        ...
        if (img5) gsap.fromTo(img5, { x: -120, rotation: -6, opacity: 0, scale: 0.9 },
          { x: 0, rotation: 0, opacity: 1, scale: 1, duration: 4, ease: "power3.out",
            scrollTrigger: { trigger: ch5, start: "top 75%" } });
        if (img5) gsap.fromTo(img5, { filter: "grayscale(1) sepia(0.4)" },
          { filter: "grayscale(0) sepia(0)", ease: "none",
            scrollTrigger: { trigger: ch5, start: "top 80%", end: "top 20%", scrub: 3 } });
        const tb5 = ch5.querySelector(".ch-text-block");
        if (tb5) gsap.fromTo(tb5, { x: 80, opacity: 0 },
          { x: 0, opacity: 1, duration: 3.5, ease: "power2.out",
      ...
      if (ch6) {
        ...
        if (img6) gsap.fromTo(img6, { y: 100, rotation: 4, opacity: 0, scale: 0.85 },
          { y: 0, rotation: 0, opacity: 1, scale: 1, duration: 4.5, ease: "power3.out",
            scrollTrigger: { trigger: ch6, start: "top 75%" } });
        if (img6) gsap.fromTo(img6, { filter: "blur(10px)" },
          { filter: "blur(0px)", duration: 3.5, ease: "power2.out",
            scrollTrigger: { trigger: ch6, start: "top 72%" } });
        const img6inner = ch6.querySelector(".story-img-inner");
        if (img6inner) gsap.fromTo(img6inner, { y: -30 }, { y: 30, ease: "none",
          scrollTrigger: { trigger: ch6, start: "top bottom", end: "bottom top", scrub: 3 } });
        const tb6 = ch6.querySelector(".ch-text-block");
        if (tb6) gsap.fromTo(tb6, { x: -80, opacity: 0 },
          { x: 0, opacity: 1, duration: 3.5, ease: "power2.out",
      ...
      if (ch7) {
        ...
        if (img7) gsap.fromTo(img7,
          { clipPath: "circle(0% at 50% 50%)", opacity: 0.5 },
          { clipPath: "circle(100% at 50% 50%)", opacity: 1, duration: 4.5, ease: "power2.inOut",
            scrollTrigger: { trigger: ch7, start: "top 72%" } });
        if (img7) gsap.fromTo(img7, { filter: "grayscale(1) sepia(0.5)" },
          { filter: "grayscale(0) sepia(0)", ease: "none",
            scrollTrigger: { trigger: ch7, start: "top 75%", end: "top 15%", scrub: 3 } });
        const img7inner = ch7.querySelector(".story-img-inner");
        if (img7inner) gsap.fromTo(img7inner, { y: -40 }, { y: 40, ease: "none",
          scrollTrigger: { trigger: ch7, start: "top bottom", end: "bottom top", scrub: 3 } });
        animTitleWords(ch7);
        animBody(ch7);
        animDecLine(ch7);
      }
      ...
      if (ch8) {
        ...
        if (bookLeft) gsap.fromTo(bookLeft, { rotationY: 0 },
          { rotationY: -70, duration: 4, ease: "power2.inOut",
            scrollTrigger: { trigger: ch8, start: "top 70%", end: "top 20%", scrub: 1.5 } });
        // Book open: right half rotates out
        const bookRight = ch8.querySelector(".book-right");
        if (bookRight) gsap.fromTo(bookRight, { rotationY: 0 },
          { rotationY: 70, duration: 4, ease: "power2.inOut",
            scrollTrigger: { trigger: ch8, start: "top 70%", end: "top 20%", scrub: 1.5 } });
        // Inner content reveals
        const bookInner = ch8.querySelector(".book-inner");
        if (bookInner) gsap.fromTo(bookInner, { opacity: 0, scale: 0.8 },
      ...
      if (ch9) {
        ...
        if (img9) gsap.fromTo(img9,
          { rotation: -180, scale: 0.3, opacity: 0 },
          { rotation: 0, scale: 1, opacity: 1, duration: 4.5, ease: "power3.out",
            scrollTrigger: { trigger: ch9, start: "top 75%" } });
        // Sepia
        if (img9) gsap.fromTo(img9, { filter: "grayscale(1) sepia(0.5)" },
          { filter: "grayscale(0) sepia(0)", ease: "none",
            scrollTrigger: { trigger: ch9, start: "top 75%", end: "top 20%", scrub: 3 } });
        const tb9 = ch9.querySelector(".ch-text-block");
        if (tb9) gsap.fromTo(tb9, { x: 80, opacity: 0 },
          { x: 0, opacity: 1, duration: 3.5, ease: "power2.out",
      ...
      if (ch10) {
        ...
        if (splitL) gsap.fromTo(splitL, { x: "0%" },
          { x: "-110%", duration: 4, ease: "power2.inOut",
            scrollTrigger: { trigger: ch10, start: "top 65%", end: "top 15%", scrub: 1.5 } });
        // Right half slides right
        const splitR = ch10.querySelector(".split-right");
        if (splitR) gsap.fromTo(splitR, { x: "0%" },
          { x: "110%", duration: 4, ease: "power2.inOut",
            scrollTrigger: { trigger: ch10, start: "top 65%", end: "top 15%", scrub: 1.5 } });
        // Inner text reveals
        const splitInner = ch10.querySelector(".split-inner");
        if (splitInner) gsap.fromTo(splitInner, { opacity: 0, y: 40, scale: 0.95 },
      ...
      if (ch11) {
        ...
        if (img11) gsap.fromTo(img11, { y: 80, opacity: 0, scale: 0.9 },
          { y: 0, opacity: 1, scale: 1, duration: 3.5, ease: "power2.out",
            scrollTrigger: { trigger: ch11, start: "top 75%" } });
        animTitleWords(ch11);
        animBody(ch11);
        const star = ch11.querySelector(".end-star");
        if (star) gsap.fromTo(star, { scale: 0, rotation: -360 },
          { scale: 1, rotation: 0, duration: 3.5, ease: "back.out(1.2)",
      ...
      function animTitleWords(parent: Element) {
        ...
        if (words.length) gsap.fromTo(words, { y: 60, opacity: 0 },
          { y: 0, opacity: 1, duration: 2.5, ease: "power2.out", stagger: 0.25,
      ...
      function animBody(parent: Element) {
        ...
        if (words.length) {
          gsap.fromTo(words, { opacity: 0, y: 8 },
            { opacity: 1, y: 0, duration: 0.3, ease: "power1.out", stagger: 0.06,
  ...
  const renderTitle = (text: string) =>
    text.split(" ").map((w, i) => (
      <span key={i} className="ch-title-word inline-block mr-3 last:mr-0">{w}</span>
    ));
  ...
  const SEC = "mx-auto w-full px-4 sm:px-6 sm:max-w-150 md:px-8 md:max-w-180 lg:max-w-240 xl:max-w-310";
  const IMG_CARD = "rounded-sm border-4 border-white shadow-[0_8px_30px_rgba(0,0,0,0.15)]";

  return (
    <>
      <div ref={sectionRef} className="bg-[#EFE9E7] text-black overflow-x-hidden">

        {/* ════ HERO ════ */}
        <section className={`${SEC} pt-24 md:pt-36 pb-16 text-center`}>
          <h2 ref={heroTitleRef} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight mb-6" style={{ perspective: "600px" }}>
          ...
          <p className="hero-sub text-lg sm:text-xl md:text-2xl text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Câu chuyện về những chiến sĩ công an đã dấn thân vào hang ổ kẻ thù, dùng chính địch để đánh địch
          </p>
          <div className="flex items-center justify-center gap-3 mt-10">
            <div className="hero-line w-24 h-[1px] bg-[#8B4513]/50 origin-left" />
            <svg viewBox="0 0 24 24" className="hero-star w-6 h-6 text-[#8B4513]" fill="currentColor">
        ...
        <article className={`ch-1 ${SEC} mb-24 md:mb-36`}>
          <StoryImage src={STORY_CHAPTERS[0].image} alt={STORY_CHAPTERS[0].imageAlt}
            className={`w-full h-[40vh] md:h-[55vh] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 0)} />
          <div className="mt-10 md:mt-14">
            <h3 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 overflow-hidden">{renderTitle(STORY_CHAPTERS[0].title)}</h3>
        ...
        <article className={`ch-2 ${SEC} mb-24 md:mb-36`}>
          <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-14">
            <div className="ch-text-block md:w-[58%]">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[1].title)}</h3>
              <div className="ch-dec-line h-[2px] w-20 bg-[#6B3410] mb-5 origin-left" />
              <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[1].text)}</p>
            </div>
            <div className="md:w-[42%]">
              <StoryImage src={STORY_CHAPTERS[1].image} alt={STORY_CHAPTERS[1].imageAlt}
                className={`w-full h-[280px] md:h-[360px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 1)} />
            </div>
          </div>
        </article>
        ...
        <article className={`ch-3 ${SEC} mb-24 md:mb-36`}>
          <div className="flex flex-col-reverse md:flex-row md:items-center gap-8 md:gap-14">
            <div className="md:w-[42%]">
              <StoryImage src={STORY_CHAPTERS[2].image} alt={STORY_CHAPTERS[2].imageAlt}
                className={`w-full h-[280px] md:h-[360px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 2)} />
            </div>
            <div className="ch-text-block md:w-[58%]">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[2].title)}</h3>
              <div className="ch-dec-line h-[2px] w-20 bg-[#4A2808] mb-5 origin-left" />
              <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[2].text)}</p>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-4 ${SEC} relative mb-24 md:mb-36`}>
          <StoryImage src={STORY_CHAPTERS[3].image} alt={STORY_CHAPTERS[3].imageAlt}
            className="w-full h-[50vh] md:h-[60vh] rounded-sm" onClick={(e) => openLightbox(e, 3)} />
          <div className="ch4-overlay absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6 sm:p-10 md:p-14 rounded-sm">
            <div className="max-w-2xl">
              <h3 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-4 text-white overflow-hidden">{renderTitle(STORY_CHAPTERS[3].title)}</h3>
              <p className="text-white/80 leading-relaxed text-base md:text-lg max-w-xl">{renderBodyWords(STORY_CHAPTERS[3].text)}</p>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-5 ${SEC} mb-24 md:mb-36`}>
          <div className="flex flex-col-reverse md:flex-row md:items-center gap-8 md:gap-14">
            <div className="md:w-[42%]">
              <StoryImage src={STORY_CHAPTERS[4].image} alt={STORY_CHAPTERS[4].imageAlt}
                className={`w-full h-[300px] md:h-[400px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 4)} />
            </div>
            <div className="ch-text-block md:w-[58%]">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[4].title)}</h3>
              <div className="ch-dec-line h-[2px] w-20 bg-[#2C5F2D] mb-5 origin-left" />
              <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[4].text)}</p>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-6 ${SEC} mb-24 md:mb-36`}>
          <div className="flex flex-col md:flex-row md:items-center gap-8 md:gap-14">
            <div className="ch-text-block md:w-[58%]">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[5].title)}</h3>
              <div className="ch-dec-line h-[2px] w-20 bg-[#4A3728] mb-5 origin-left" />
              <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[5].text)}</p>
            </div>
            <div className="md:w-[42%]">
              <StoryImage src={STORY_CHAPTERS[5].image} alt={STORY_CHAPTERS[5].imageAlt}
                className={`w-full h-[300px] md:h-[400px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 5)} />
            </div>
          </div>
        </article>
        ...
        <article className={`ch-7 ${SEC} mb-24 md:mb-36`}>
          <StoryImage src={STORY_CHAPTERS[6].image} alt={STORY_CHAPTERS[6].imageAlt}
            className={`w-full h-[40vh] md:h-[55vh] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 6)} />
          <div className="mt-10 md:mt-14">
            <h3 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[6].title)}</h3>
            <div className="ch-dec-line h-[2px] w-24 bg-[#6B3410] mb-5 origin-left" />
            <p className="text-gray-700 leading-relaxed text-base md:text-lg max-w-3xl">{renderBodyWords(STORY_CHAPTERS[6].text)}</p>
          </div>
        </article>

        {/* ════ CH8: BOOK OPEN — image splits like opening a book ════ */}
        <article className={`ch-8 ${SEC} mb-24 md:mb-36`}>
          <div className="relative w-full h-[45vh] md:h-[55vh] mb-10 md:mb-14" style={{ perspective: "1200px" }}>
            {/* Left half of the "book" */}
            <div className="book-left absolute inset-0 w-1/2 overflow-hidden origin-right" style={{ transformStyle: "preserve-3d" }}>
              <div className="relative w-[200%] h-full">
                <Image src={STORY_CHAPTERS[7].image} alt={STORY_CHAPTERS[7].imageAlt} fill sizes="80vw" className="object-cover" />
              </div>
            </div>
            {/* Right half of the "book" */}
            <div className="book-right absolute inset-0 left-1/2 w-1/2 overflow-hidden origin-left" style={{ transformStyle: "preserve-3d" }}>
            ...
            <div className="book-inner absolute inset-0 flex items-center justify-center bg-[#EFE9E7]">
              <div className="text-center px-6 max-w-2xl">
                <h3 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[7].title)}</h3>
                <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[7].text)}</p>
              </div>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-9 ${SEC} mb-24 md:mb-36`}>
          <div className="flex flex-col-reverse md:flex-row md:items-center gap-8 md:gap-14">
            <div className="md:w-[45%]">
              <StoryImage src={STORY_CHAPTERS[8].image} alt={STORY_CHAPTERS[8].imageAlt}
                className={`w-full h-[300px] md:h-[400px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 8)} />
            </div>
            <div className="ch-text-block md:w-[55%]">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 overflow-hidden">{renderTitle(STORY_CHAPTERS[8].title)}</h3>
              <div className="ch-dec-line h-[2px] w-20 bg-[#2C3E50] mb-5 origin-left" />
              <p className="text-gray-700 leading-relaxed text-base md:text-lg">{renderBodyWords(STORY_CHAPTERS[8].text)}</p>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-10 ${SEC} mb-24 md:mb-36`}>
          <div className="relative w-full h-[50vh] md:h-[60vh] overflow-hidden rounded-sm">
            ...
            <div className="split-left absolute inset-0 w-1/2 overflow-hidden">
              <div className="relative w-[200%] h-full">
                <Image src={STORY_CHAPTERS[9].image} alt={STORY_CHAPTERS[9].imageAlt} fill sizes="80vw" className="object-cover" />
              </div>
            </div>
            {/* Right split */}
            ...
            <div className="split-inner absolute inset-0 flex items-center justify-center bg-[#EFE9E7]">
              <div className="text-center px-6 max-w-3xl">
                ...
                <p className="text-gray-700 leading-relaxed text-base md:text-lg max-w-2xl mx-auto">{renderBodyWords(STORY_CHAPTERS[9].text)}</p>
              </div>
            </div>
          </div>
        </article>
        ...
        <article className={`ch-11 ${SEC} mb-20 md:mb-28 text-center`}>
          <div className="max-w-md mx-auto mb-8">
            <StoryImage src={STORY_CHAPTERS[9].image} alt={STORY_CHAPTERS[9].imageAlt}
              className={`w-full h-[240px] md:h-[300px] ${IMG_CARD}`} onClick={(e) => openLightbox(e, 9)} />
          </div>
          ...
          <div className="end-star flex justify-center mt-14">
            <svg viewBox="0 0 24 24" className="w-10 h-10 text-[#8B4513]" fill="currentColor">
              <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
            </svg>
          </div>
        </article>
      ...
      <ImageLightbox data={lightboxData} sourceRect={lightboxRect}
        onClose={() => { setLightboxData(null); setLightboxRect(null); }} />
    </>
  );
}